    ...
    <div class="col-lg-6">
        ...
        <div class="form-group">
            ...
            <select name="organization" class="form-control" required>
                ...
                <option {{ (@$item->hosting_organiation==$row->id)?'selected':'' }} value="{{$row->id}}">
        ...
        <div class="form-group">
            ...
            <select name="thematic_areas[]" class="form-control filter-multi-select" required multiple>
                ...
                <option {{ @$item->thematic_areas &&
          (@$item->thematic_areas->contains('thematic_area_id',$row->id))?'selected':'' }} value="{{$row->id}}">
        ...
        <div class="form-group">
            ...
            <select name="approval_authority" class="form-control" required>
                ...
                <option {{ (@$auth->id==@$item->approval_authority_id)?'selected':'' }} value="{{$auth->id}}">
                    {{$auth->authority_name}}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="form-group">
            ...
            <select name="uitool" class="form-control" required>
                ...
                <option {{ (@$tool->id==@$item->ui_tool_id)?'selected':'' }} value="{{$tool->id}}">{{$tool->tool_name}}
                </option>
                @endforeach
            </select>
        </div>
    ...
    <div class="col-lg-6">
        ...
        <div class="form-group">
            ...
            <select name="contact" class="form-control filter-multi-select" multiple>
                ...
                <option value="{{$contact->id}}" {{(in_array($contact->id,item_contacts(@$item->id)))?'selected':''}}>
                    {{@$contact->name}}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="form-group">
            ...
            <select name="dev_entity" class="form-control" required>
                ...
                <option {{ (@$entity->id==@$item->dev_entity_id)?'selected':'' }} value="{{$entity->id}}">
                    {{$entity->entity_name}}</option>
                @endforeach
            </select>
        </div>
